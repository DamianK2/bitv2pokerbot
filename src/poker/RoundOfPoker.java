...
public class RoundOfPoker {
    ...
    public void play(GameOfPoker game)
    {
        ...
        for (PokerPlayer player : this.players)
            game.updateGameMessage(player.getName() + " has " + player.getCoinsBalance() + " coins");

        // CHECK IF ANY PLAYER CAN OPEN
        boolean canOpen = false;
        for (PokerPlayer player : this.players)
            if (player.canOpenBet()) {
                game.updateGameMessage(player.getName() + ": I can open");
                canOpen = true;
            }
            else
                game.updateGameMessage(player.getName() + ": I cannot open");

        if (!canOpen) {
            game.updateGameMessage("Cannot open game.");
            return;
        }

        game.updateGameMessage("You hand is:");
        ...
        for (PokerPlayer player : this.players)
            if (player.isHuman()) {
                game.updateGameMessage("Your new hand:");
                game.updateGameMessage(player.getHand());
            }
            ...
            for (int i = 0; i < this.players.size(); i++) {
                ...
                if (!fold[i]) {
                    ...
                    if (checkOpen == 0 && firstOpen) {
                        game.updateGameMessage(players.get(i).getName() + ": I open with " + this.currentBet + " chip!");
                        currentPot += this.currentBet;
                        checkOpen = 1;
                    }
                        ...
                        if (players.get(i).isHuman() && !fold[i]) {
                            ...
                            // CHECK IF THE PLAYER DIDN'T RAISE THE BET AND THE BETTING ISN'T THE OPENING BET THEN FOLD
                               ...
                               //System.out.println("Would you like to fold (y/n)? ");
                                fold[i] = true;
                                game.updateGameMessage(players.get(i).getName() + ": I fold ");
                            }
                        }
